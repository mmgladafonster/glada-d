...
class SecurityAlertManager {
  ...
  private async sendEmailAlert(alert: SecurityAlert, eventCount: number): Promise<void> {
    try {
      ...
      if (!resend) {
        throw new Error('Email service not configured - RESEND_API_KEY missing or invalid')
      }
...
Generated by Glada Fönster Security Monitoring System
  ...
  // Update alert configuration (thresholds merge partially)
  updateConfig(
    newConfig: Partial<Omit<AlertConfig, 'alertThresholds'>> & {
      alertThresholds?: Partial<AlertConfig['alertThresholds']>
    }
  ): void {
    this.config = {
      ...this.config,
      ...
      alertThresholds: newConfig.alertThresholds
        ? { ...this.config.alertThresholds, ...newConfig.alertThresholds }
